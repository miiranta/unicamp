"""GELU308 – Log-Space Additive Gate (Different Gradient Landscape).

MOTIVATION: gelu211 computes gates as PRODUCTS: gate_in × gate_out × gate_cos.
In log-space this becomes ADDITION, which has different gradient behaviour:
each gate contributes additively to the log-output, so gradients from different
gate components are independent and don't interact multiplicatively.

LOG-SPACE GATE COMPUTATION:
    log_g_in  = log_clamp(gate_in(z_in), ε, max)    [from asymmetric z-score]
    log_g_out = log_clamp(gate_out(z_out), ε, max)  [from symmetric z-score]
    log_g_cos = -τ * cos(out, ema_dir)               [cosine dissimilarity]

    log_gate = log_g_in + log_g_out + log_g_cos
    output   = out * exp(log_gate.clamp(-3.0, 2.3))  [clamp in log-space: ≈ 0.05 to 10]

WHY DIFFERENT:
    - Each additive component in log-space has INDEPENDENT gradient magnitude
    - No "vanishing gate signal" when one gate is near 0
    - More numerically stable than multiplying three gates
    - Allows the model to learn which gate component matters most

CAUSALITY: same batch-level EMA, no future leakage.

PARAMS: logit_decay, log_tau, log_beta_up, log_beta_dn, log_gamma,
        log_beta_out, log_gamma_out  (7, same as gelu211)
"""
...
class GELU308(nn.Module):
    """Log-space additive combination of in/out/cosine gate components."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Log-space addition
        log_g_in  = gate_in.log()                    # (B, T, D)
        log_g_out = gate_out.log()                   # (B, T, D)

        with torch.no_grad():
            out_n    = F.normalize(out.detach(), dim=-1)
            ema_n    = F.normalize(self._ema_out_dir, dim=0).view(1, 1, D)
            cos_sim  = (out_n * ema_n).sum(-1).clamp(-1, 1)
            log_g_cos = (-tau * cos_sim).unsqueeze(-1)   # (B, T, 1)

        log_gate = log_g_in + log_g_out + log_g_cos     # (B, T, D)
